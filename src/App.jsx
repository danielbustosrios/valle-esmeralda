...
const WORLD_END_LEVELS=new Set([11,17,21,26,31,36,41,46,60,67,68]);
...
export default function App(){
 ...
 const [levelIndex,setLevelIndex]=useState(()=>{const requested=Math.max(1,Number(new URLSearchParams(location.search).get('level'))||1),exact=LEVELS.findIndex(item=>item.id===requested);if(exact>=0)return exact;const next=LEVELS.findIndex(item=>item.id>requested);return next>=0?next:LEVELS.length-1;}),[height,setHeight]=useState(0),[direction,setDirection]=useState(0);
 const level=LEVELS[levelIndex];
 const worldComplete=WORLD_END_LEVELS.has(level.id);
 ...
 const advanceAfterLevel=()=>worldComplete?location.href='?view=map':levelIndex<LEVELS.length-1?reset(levelIndex+1):reset(0);
 ...
 if(level.darkBoss)return <DarkBossLevel level={level} onNext={advanceAfterLevel}/>;
 if(level.bomber)return <BomberLevel key={level.id} level={level} onNext={advanceAfterLevel}/>;
 if(level.pythagoreanBoss)return <PythagoreanBossLevel key={level.id}/>;
 if(level.islandPuzzle)return <IslandPuzzleLevel key={level.id} level={level} onNext={advanceAfterLevel}/>;
 if(level.maze)return <MazeLevel level={level} levels={LEVELS} worldComplete={worldComplete} onNext={advanceAfterLevel}/>;
 return level.cannonCoordinate?<CannonCoordinateLevel level={level} levels={LEVELS} worldComplete={worldComplete} onSelectLevel={reset} onNext={advanceAfterLevel}/>:level.coordinate?<CoordinateLevel level={level} levels={LEVELS} worldComplete={worldComplete} onSelectLevel={reset} onNext={advanceAfterLevel}/>:level.logic?<LogicLevel level={level} levels={LEVELS} worldComplete={worldComplete} onSelectLevel={reset} onNext={advanceAfterLevel}/>:<main><header><a className="brand" href="?view=map" aria-label="Abrir mapa de mundos"><span>VALLE</span> ESMERALDA<small>UNA AVENTURA POR DESCUBRIR</small></a><div className="chapter"><span>CAPÍTULO 01</span><strong>{level.name}</strong></div><div className="inventory"><span title="Estrellas">★ <b>{level.boss?bossStars:won?1:0}</b><small>/ {level.boss?3:1}</small></span><span title="Cristales">◆ <b>{won?1:0}</b><small>/ 1</small></span><span className="energy" aria-label={`${hearts} corazones`}>{level.boss?Array.from({length:2},(_,i)=>i<hearts?'♥':'♡').join(' '):'♥ ♥ ♥'}</span></div><HubLinks guest={activeUser.isGuest}/><button className="menu-button" onClick={()=>setMenu(true)} aria-label="Abrir menú">☰</button></header>
 ...
 {won&&<div className="victory" role="dialog" aria-modal="true" aria-labelledby="win-title"><div className="win-star">{level.boss?'★'.repeat(bossStars):'★'}</div><small>{worldComplete?'MUNDO COMPLETADO':level.boss?'CAPÍTULO COMPLETADO':'NIVEL COMPLETADO'}</small><h1 id="win-title">{level.boss?'¡Escapaste del derrumbe!':stage===5?'¡Precisión perfecta!':'¡El cristal es tuyo!'}</h1><p>{level.boss?`Resististe ${level.survivalSeconds} segundos, esquivaste ${bossHits} rocas y conservaste ${hearts} corazones.`:stage===5?'¡Atravesaste el aro y golpeaste la diana con el mismo disparo!':stage===4?'¡Diana activada! Ya has dado tus primeros pasos con a, b y c.':stage===3?'¡Los dos Saqueadores han sido derrotados!':'El valle vuelve a brillar. Buen disparo.'}</p><div className="rewards">★ {level.boss?bossStars:1} estrella{level.boss&&bossStars!==1?'s':''} <span>◆ 1 cristal</span></div><button autoFocus onClick={()=>{saveLevelCompletion(level.id,level.boss?bossStars:1);advanceAfterLevel();}}>{worldComplete?'MUNDO COMPLETADO · VOLVER AL MAPA →':levelIndex<LEVELS.length-1?`Nivel ${LEVELS[levelIndex+1].id} →`:'Volver a esquivar ↻'}</button></div>}
