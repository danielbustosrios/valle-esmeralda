...
export default function World({level=LEVEL,points,projectile,won,hit,pending,defeated=[],barrelUsed=false,gateActive=false,shooting,missFeedback,paused,dodgeState={x:600,jump:0,rocks:[],invulnerable:0},onReactionEnd}) {
  ...
  return <svg className="world" viewBox="0 0 1200 800" role="img" aria-label={level.boss?"Valle Esmeralda durante un derrumbe: el protagonista esquiva muchas rocas":"Valle Esmeralda: protagonista, cañón, ruinas, cascadas y objetivos"}>
    ...
    {level.boss&&<g className={`dodge-scene ${paused?'is-paused':''}`}>
      {dodgeState.rocks.map(rock=>{const fallTime=Math.max(0,(575-rock.y)/Math.max(1,rock.vy)),landingX=Math.max(45,Math.min(1155,rock.x+rock.vx*fallTime)),warningWidth=Math.max(22,rock.r*.72),warningHeight=Math.max(7,rock.r*.17);return <g key={`warning-${rock.id}`} className="rock-warning" transform={`translate(${landingX} 574)`} opacity={Math.max(.18,Math.min(.88,(rock.y+180)/560))} aria-hidden="true"><ellipse rx={warningWidth} ry={warningHeight}/><path d={`M-${warningWidth+10} 0h12M${warningWidth-2} 0h12M0 -${warningHeight+8}v8`}/><circle r="3"/></g>})}
