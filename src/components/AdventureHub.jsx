...
const ITEMS=[
  ...
  {id:'five',name:'Reto por un 5,0',description:'Desbloquea un reto especial. La nota depende de resolverlo y de la aprobación del profesor.',cost:40,currency:'◆',icon:'🏅',academic:true},
  {id:'half',name:'Impulso de +0,5',description:'Beneficio individual o para un compañero, sujeto a una actividad y a la aprobación del profesor.',cost:300,currency:'●',icon:'+0,5',academic:true}
];
...
export default function AdventureHub({view='map',levels,user}){
  ...
  return <main className="adventure-hub"><header><a className="brand" href="?view=map"><span>VALLE</span> ESMERALDA<small>UNA AVENTURA POR DESCUBRIR</small></a><div className="chapter"><span>CENTRO DE LA AVENTURA</span><strong>{title}</strong></div><div className="inventory"><span>★ <b>{stars}</b></span><span>◆ <b>{balance}</b><small> disponibles</small></span></div></header>
    ...
    {view==='shop'&&<section className="shop-screen scene-shop"><div className="shop-scene"><img src="./art/profe-dani-shop.webp" alt="Profe Dani B atendiendo su tienda detrás de un mostrador de madera"/><img className="shop-visiting-hero" src="./art/hero-dodge.webp" alt="El protagonista entrando a la tienda"/><div className="shop-scene-title"><small>TALLER Y TIENDA</small><strong>Profe Dani B</strong></div><div className="shop-scene-wallet"><small>TU BOLSA</small><strong>◆ {balance}</strong></div><div className="shop-scene-actions"><button className="shop-talk-button" onClick={reopenShopDialogue} disabled={showShopDialogue}><span>💬</span> {showShopDialogue?'CONVERSANDO':'HABLAR CON DANI B'}</button><button className="shop-ranking-button" onClick={()=>setShowRanking(true)}><span>🏆</span> RANKING</button></div><div className={`shop-starter-progress${fourComplete?' complete':''}`}><small>CUATRO CAMINOS</small><strong>{starterCompleted}/4</strong></div>{showShopDialogue&&<ShopDialogue key={dialogueRound} lines={[{speaker:'PROFE DANI B',text:starterAdvice},{speaker:'PROTAGONISTA',text:'Entonces puedo elegir por dónde comenzar, pero debo superar los cuatro caminos para continuar.'},{speaker:'PROFE DANI B',text:'Exactamente. Cada mundo entrena una capacidad diferente y todas serán necesarias más adelante.'},{speaker:'PROFE DANI B',text:teacherAdvice},{speaker:'PROFE DANI B',text:'Las mejoras de esta tienda pueden acompañarte, pero el verdadero avance lo consigues resolviendo los desafíos.'}]} onComplete={finishShopDialogue}/>}</div><header className="shop-catalog-title"><small>PRÓXIMAS RECOMPENSAS</small><h1>Canjea lo que conquistes en el Valle</h1><p>Esta es una vista previa. Los precios, las monedas y las reglas se configurarán antes de habilitar los canjes.</p></header><div className="shop-grid">{ITEMS.map(item=><article className={`shop-item ${item.id}${item.academic?' academic-reward':''}`} key={item.id}><div className="item-preview"><span>{item.icon}</span></div><small>PRÓXIMAMENTE</small><h2>{item.name}</h2><p>{item.description}</p><div className="future-price"><span>{item.currency} {item.cost}</span><em>{item.academic?'REQUIERE APROBACIÓN':'AÚN NO DISPONIBLE'}</em></div><button disabled>PRÓXIMAMENTE · NO DISPONIBLE</button></article>)}</div><p className="shop-note">Las recompensas académicas nunca se aplicarán automáticamente: requerirán una actividad definida y la aprobación del profesor.</p>{showRanking&&<StudentLeaderboard user={user} onClose={()=>setShowRanking(false)}/>}</section>}
  </main>;
}
