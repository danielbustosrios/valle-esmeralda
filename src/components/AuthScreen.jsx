...
import {isSupabaseReady,loginStudent,registerStudent,sendRecovery,updatePassword} from '../game/supabaseAuth.js';

const TITLES={login:['BIENVENIDO DE NUEVO','Continúa tu aventura'],register:['NUEVO EXPLORADOR','Crea tu perfil'],recover:['RECUPERAR ACCESO','Volver al camino'],reset:['NUEVA CONTRASEÑA','Recupera tu camino']};
...
export default function AuthScreen({onAuthenticated,recovering=false,onRecovered}){
  ...
  const [form,setForm]=useState({firstName:'',lastName:'',course:'',email:'',password:'',confirm:'',remember:true});
  ...
  const submit=async event=>{
    ...
    if(mode!=='reset'&&!/^\S+@\S+\.\S+$/.test(form.email)){setError('Escribe un correo válido.');return;}
    if(mode!=='recover'&&form.password.length<6){setError('La contraseña debe tener al menos 6 caracteres.');return;}
  ...
  return <main className="auth-page">
    ...
    <section className="auth-panel"><div className="auth-card"><div className="auth-mobile-brand">VALLE <b>ESMERALDA</b></div><small>{kicker}</small><h2>{title}</h2>
      <p className="auth-intro">{mode==='login'?'Ingresa para conservar tu lugar en el mapa.':mode==='register'?'Crea la identidad que usarás durante la aventura.':'Indica el correo asociado a tu perfil.'}</p>
      <form onSubmit={submit}>
        ...
        {mode!=='reset'&&<label>Correo electrónico<input name="email" value={form.email} onChange={update} type="email" inputMode="email" autoComplete="email" placeholder="nombre@correo.com"/></label>}
        {mode!=='recover'&&<label>{mode==='reset'?'Nueva contraseña':'Contraseña'}<input name="password" value={form.password} onChange={update} type="password" autoComplete={mode==='login'?'current-password':'new-password'} placeholder="Mínimo 6 caracteres"/></label>}
        ...
        {error&&<strong className="auth-error" role="alert">{error}</strong>}{notice&&<div className="auth-notice" role="status">✦ {notice}</div>}<button className="auth-submit" disabled={busy||!isSupabaseReady}>{busy?'CONECTANDO…':mode==='login'?'ENTRAR A LA AVENTURA →':mode==='register'?'CREAR PERFIL →':mode==='reset'?'GUARDAR CONTRASEÑA →':'ENVIAR ENLACE →'}</button>
      </form>
      {mode!=='reset'&&<div className="auth-switches">{mode!=='login'&&<button onClick={()=>changeMode('login')}>Ya tengo un perfil</button>}{mode!=='register'&&<button onClick={()=>changeMode('register')}>Crear usuario</button>}{mode!=='recover'&&<button onClick={()=>changeMode('recover')}>Olvidé mi contraseña</button>}</div>}
